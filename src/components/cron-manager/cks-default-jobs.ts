 ...
 *   - src/components/cron-manager/CronJobForm.tsx (buildFriendlyPayload)
 *   - src/routes/api/cron/upsert.ts (cron.add format con sessionTarget: 'main')
 ...
 *
 * `sessionTarget` está hardcoded a `'main'` en cron.add. Todos los agentTurns
 * entran por el maestro, que delega al leaf correcto vía routing semántico
 * definido en `agents.list[main].systemPromptOverride`:
